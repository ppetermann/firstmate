...
- `ocr-bot-review` - load on any `ocr-pr <owner/repo>#<n> <head_sha>` or `ocrbot-error <detail>` check wake; relevant only when the OCR review bot is armed (`config/ocr-bot/`).
- `ocr-delegate-review` - load only when the captain explicitly requests an OpenCodeReview (OCR) review round for a specific task; never as a default or automatic gate.
