...
#                RULE-DELIMITED variant: claude draws that row between two
#                horizontal rules and inlines its in-progress todo into the TOP
#                rule on a wide pane, leaving the BOTTOM rule unmatched below
#                the composer. An unmatched rule below a candidate normally
#                proves it stale, so that shape retired claude's own live
#                composer; it is rescued only for the bare shape, only when the
#                rule is the row's immediate successor, and only against a live
#                non-pi agent identity (_fm_composer_classify_bare_rule_delimited).
...
# placeholder counting as unsubmitted text, so an idle OpenCode composer read
# `pending` and no submit to it could ever be confirmed.
...
_fm_composer_scan_screen() {  # <plain-screen> <cursor-or-empty> [extract-wrap]
  ...
  local leftbar_start=-1 leftbar_indent='' leftbar_len=0 pi_open=-1 pi_lines=0 pi_max
  ...
  while IFS= read -r line; do
    ...
    # A LONE bar-led row is ordinary output - a pipe, a table border, a TUI
    # frame - not a drawn composer. Only an aligned repeat of the same bar at the
    # same indent proves a rail, so the run must reach two rows before it is
    # published as a candidate. Without that floor a single `┃` row anywhere on
    # screen classifies as an empty composer, and the away-mode injector types
    # escalations into whatever drew it; that is the same fail-closed boundary
    # the bare SHELL glyph rule holds, applied to the left-bar shape.
    case "$trimmed" in
      '┃'*'┃') leftbar_start=-1; leftbar_indent=''; leftbar_len=0 ;;
      '┃'*)
        if [ "$leftbar_start" -lt 0 ] || [ "$indent" != "$leftbar_indent" ]; then
          leftbar_start=$row; leftbar_indent=$indent; leftbar_len=1
        else
          leftbar_len=$((leftbar_len + 1))
        fi
        if [ "$leftbar_len" -ge 2 ]; then
...
_fm_composer_select_cursorless() {
  ...
  FM_COMPOSER_SELECTED_RULE_RESCUE=0
  ...
  if [ "$FM_COMPOSER_SCAN_PI_PAIR_FOUND" = 0 ] \
    ...
    # An unmatched separator BELOW the candidate normally proves that candidate
    # stale. It proves nothing of the sort when the separator is the bare
    # composer's OWN closing rule: claude draws a RULE-DELIMITED composer
    # (rule / `❯` / rule), so its closing rule always sits directly below the
    # live composer row. While both rules are plain `─` runs the pair completes
    # and the row is enclosed, but claude inlines the in-progress todo into the
    # TOP rule once the pane is wide enough (` Run tests ` at 107 columns,
    # rendered as separate rows below ~80), which stops that rule being a
    # separator and left the closing rule unmatched - retiring claude's own live
    # composer as `unknown`. Measured on the captain's real pane (claude
    # 2.1.226 on herdr 0.8.0): every away-mode escalation deferred to the
    # max-defer wedge alarm while nothing was actually wrong.
    #
    # Two structural signals are required here and a third (identity) is
    # required by the verdict below, deliberately conjunctive rather than
    # alternative because the two failure directions are not symmetric:
    # refusing a genuine composer only defers an escalation (the max-defer
    # alarm surfaces it), while accepting a stale one types into whatever now
    # owns the pane, where text RUNS.
    #   - shape: the candidate is the BARE agent-glyph row. That is the only
    #     shape with live evidence behind it; a box or left-bar candidate
    #     followed by a rule keeps precisely the refusal it had before, so this
    #     branch can never turn an injection-REFUSING verdict into an
    #     injection-AUTHORIZING one for a shape it has no evidence about.
    ...
    if [ "$FM_COMPOSER_SELECTED_KIND" = bare ] \
      ...
      FM_COMPOSER_SELECTED_RULE_RESCUE=1
    else
      FM_COMPOSER_SELECTED_KIND=
      return 1
    fi
...
EOF
  ...
  case "$FM_COMPOSER_SELECTED_KIND" in
    ...
    box)
      ...
      ;;
    bare)
      if [ "$FM_COMPOSER_SELECTED_RULE_RESCUE" = 1 ]; then
        _fm_composer_classify_bare_rule_delimited "$screen" "$styled" \
          "$has_identity" "$identity" "$FM_COMPOSER_SCAN_BARE_ROW"
...
# _fm_composer_classify_bare_rule_delimited: the third signal of the
# rule-delimited rescue selected in _fm_composer_select_cursorless. The two
# structural signals proved the unmatched rule is the bare composer's own
# closing rule; only a live native agent record proves the composer is still
# owned by an agent rather than by the shell that replaced it. Scrollback left
# by an EXITED agent keeps both the `❯` row and its rule but loses that record,
# so it stays `unknown` exactly as before - which is what preserves the
# dead-shell refusal here. Measured against a real exit (claude 2.1.226 on
# herdr 0.8.0, isolated lab): herdr DROPPED the record within 0.05s of the pane
# returning to its shell. A pi identity is excluded because a pi pane's rule
# genuinely is a pi separator, so its own shape owner must judge it.
_fm_composer_classify_bare_rule_delimited() {  # <screen> <styled> <has-identity> <identity> <bare-row>
  local screen=$1 styled=$2 has_identity=$3 identity=$4 row=$5 agent
  if [ "$has_identity" != 1 ]; then
    printf 'unknown'
    return 0
