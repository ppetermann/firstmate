...
A crew that declares `paused:` for a known external wait is separately absorbed while idle - including while an authoritative run-step is attributed and its agent is not confidently dead - and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
A confidently dead agent under an attributed run falls back to the provably-working wedge path instead, so an orphaned run behind an exited worker still escalates.
