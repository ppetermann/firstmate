...
Stopping a worker is signalled to its whole process group, so the worker treats every stop signal after the first as redundant and finishes the shutdown it already started, which is what guarantees it releases its ownership lock for the next worker instead of abandoning one no replacement could reclaim.
