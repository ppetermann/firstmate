...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk, holder liveness, and the daemon-hosted stable-token fallback) for fm-lock.sh and the Claude Stop auto-arm |
...
| `fm-signal-wait-lib.sh`  | Single owner of signal-responsive waiting: sleeps and child waits a trapped signal interrupts at once, plus bounded child shutdown |
