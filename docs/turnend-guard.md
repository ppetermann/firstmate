...
- The global hook is not permanent: teardown of the last Kimi crew task empties that registry and then removes the hook, a relaunch away from Kimi retires through the same path, and `remove` refuses while the registry still holds tokens; [`configuration.md`](configuration.md#harness-support) owns that spawn and teardown lifecycle for Kimi and grok alike.
- The spawn's pre-flight is the installer's read-only `validate`, its token plus hook files land through `mint` in one step, and both mint and retirement hold the per-harness registry lock, so a retirement racing a spawn can neither destroy the new token nor strip the hook it needs.
...
`tests/fm-kimi-harness.test.sh` covers the separate Kimi crew hook's format preservation, idempotence, refusal cases, token guard, spawn registration, teardown cleanup, reinstallation on a fresh spawn after the hook was retired, the read-only pre-flight on a retired home, and retirement serializing with a locked mint.
`tests/fm-teardown.test.sh` covers last-task retirement of the grok and Kimi global hooks, preservation while another task is still registered, and the Kimi `remove` refusal on a non-empty registry; `tests/fm-grok-harness.test.sh` adds grok's spawn, last-task teardown, respawn path, retirement waiting on the registry lock, and the spawn-side hook install holding that lock; `tests/fm-control-relaunch.test.sh` adds relaunching away from grok or Kimi retiring the prior harness's global hooks.
