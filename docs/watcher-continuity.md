...
Self-recognition itself is owned by `bin/fm-session-lock-lib.sh`'s `fm_session_lock_owned_by_self`: ordinary OS process ancestry first, falling back to a stable per-session identity token for a Claude daemon-hosted session whose hook subprocesses share no ancestry path with the process that wrote the lock.
...
Under the away-supervisor daemon (`state/.afk` present) the watcher restarts as a one-shot in a tight loop, so the resurface claims its recovery generation (advancing the marker `pending:downtime` to `pending:handling`) before emitting that wake; a rapid restart then resolves to `action=wait` instead of re-firing the same generation.
The ordinary persistent-arm path leaves the marker `pending:downtime` after emitting, so its arm layer can establish a successor and transition handling via `--handling-delivered`; each crash-replay re-arm re-surfaces the same generation until that successor is confirmed.
Either way each genuine new downtime generation gets exactly one resurface, and the post-handling drain acknowledges the generation as before.
...
An interrupted arm always completes its own stop.
...
`tests/fm-watch-arm.test.sh` covers durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, an interrupted arm that still stops a watcher which ignores SIGTERM, a watcher close inside the handling window that must leave the printed acknowledgement valid, and the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy.
`tests/fm-wake-daemon-lifecycle-e2e.test.sh` covers the away-supervisor daemon's one-shot watcher restart cycle, including a rearm-resurface idempotent per recovery generation so the tight restart loop cannot re-fire it, while a fresh downtime generation still resurfaces once.
