...
# Idle matching applies only to an UNSTYLED capture (cmux, orca). On a styled
# capture the harness's own placeholder is de-emphasised and already removed by
# ghost stripping, so a placeholder-LOOKALIKE that survives styling is real typed
# text and must never read empty - that half is asserted below too, because it is
# what stops the injector typing over a steer that merely resembles the hint.
test_idle_placeholder_is_empty_in_non_utf8_locale() {
  local idle='^Type a message\.\.\.$' loc g out
  for loc in C POSIX; do
    for g in '❯' '›' '⟩' '>'; do
      # bordered=1, proven placeholder position, unstyled capture.
      out=$(classify_in_locale "$loc" 1 "$g Type a message..." "$idle" sensitive "$g Type a message..." 1 0)
      [ "$out" = empty ] \
        || fail "under LC_ALL=$loc the idle placeholder after '$g' should read empty on an unstyled capture, got '$out' (a byte-counting glyph strip leaves the glyph's trailing bytes as leading content and the post-strip idle match can never fire)"
      # The same row on a STYLED capture survived ghost stripping, so it is real
      # input, not the harness's hint.
      out=$(classify_in_locale "$loc" 1 "$g Type a message..." "$idle" sensitive "$g Type a message..." 1 1)
      [ "$out" != empty ] \
        || fail "under LC_ALL=$loc a placeholder-lookalike that survived ghost stripping on a styled capture must not read empty, got '$out'"
    done
    out=$(classify_in_locale "$loc" 1 '❯ fix findings 1 and 3' "$idle" sensitive '❯ fix findings 1 and 3' 1 0)
    [ "$out" = pending ] \
