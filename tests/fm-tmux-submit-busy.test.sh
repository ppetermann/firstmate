...
test_busy_pane_unknown_stays_unknown() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_COMPOSER="$composer" FM_FAKE_PANE_BUSY=1 \
    ...
    fm_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  # No idle baseline is supplied (a direct fm_tmux_submit_enter_core caller), so
  # the turn-started conversion has no transition evidence to stand on and the
  # unreadable verdict must survive untouched.
  [ "$(cat "$vfile")" = unknown ] \
    || fail "a busy pane must not convert an unsafe composer to empty without a proven idle baseline, got '$(cat "$vfile")'"
